...
| `wcd_client.py` | 499 | WCD HTTP API 客户端（兼容 WeFlowClient 接口） |
| `sync.py` | 332 | 同步编排器：health check → contacts → conversations → 私聊消息 → moments |
...
| `db_init.py` | 307 | 数据库初始化（建表、索引、迁移） |
| `screenshot_import.py` | 575 | 截图 OCR 导入主流程 |
| `screenshot_parser.py` | 299 | 截图消息解析（识别发送者、时间、内容） |
| `ocr_engine.py` | 163 | RapidOCR 封装（ONNX Runtime，带 MD5 缓存） |
...
| `emojiMd5` | (构造 XML) | 贴纸 → `<msg><emoji md5="..."/></msg>` |
...
run_sync(config, mode='incremental')  # 默认增量，仅私聊
    ...
    │      ├─ 遇到 500 错误自动缩小 limit 重试（500→200→100）
    │      └─ 更新 checkpoint
...
**仅私聊**：消息同步默认只处理 `type='private'` 的会话（wxid_ 开头的个人聊天）。群聊（@chatroom）和公众号（gh_）不会同步消息。
...
`sync(meta_only=True)` 只同步联系人和会话列表，不同步消息。用于快速刷新联系人搜索索引（约 1 秒）。
...
Step 2: 截图放到 data/input/<名字>/ 目录下
    │
Step 3: prepare_import(conn, wxid, screenshot_dir, platform)
...
| `contact_merges` | merge_wxid (TEXT) | 账号合并记录 |
...
4. **500 错误重试**：API 偶尔返回 500，sync_messages 会自动缩小 limit 重试 3 次。
5. **会话类型判断**：`sync_conversations.py` 不信任 API 的 type 字段，根据 session_id 模式判断（wxid_ 开头 = private，@chatroom = group）。
