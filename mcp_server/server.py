...
mcp.tool(
    ...
    description="【微信连续发消息 v4】向同一联系人连续发送多条消息（第一条走完整流程，后续跳过搜索直接发送）。"
               "⚠️ 会抢鼠标：执行期间会移动鼠标并占用键鼠，调用前 Agent 应当口头提醒用户"
               "'即将连续发送微信消息，执行期间请勿操作鼠标键盘'（不需要阻塞等待确认，说出来即可）。"
               "前置条件：微信已运行并登录，联系人头像模板已存放在 data/avatars/<wxid>.jpg。"
               "参数：name（联系人标识符：微信号/wxid/昵称/备注名 均可），messages（消息列表，每条独立发送），"
               "urgent（紧急模式，True 时绕过回复冷却校验）。"
...
mcp.tool(
    ...
    description="【用户图片搜索】模糊搜索用户图片库，返回匹配图片的描述和绝对路径。"
               ...
               "category（分类精确过滤：子文件夹名，例如'category-a'，不传返回所有）/ "
               "limit（最多返回数，默认 20）。"
               "返回包含 categories_summary（所有分类概览，供 agent 浏览）+ results（匹配图片列表，含 description/keywords/suitable_when/usage_tips）。"
               "使用场景：按关键词搜索→keywords=['topic'] / 想看有什么图片→不传 keywords / 看特定分类→category='category-a'。",
    annotations={"readOnlyHint": True},
